...
def conversation1_B():  # sourcery skip: extract-duplicate-method, remove-redundant-if
    ...
    if all(choice in ("a", "c") for choice in counter_All):
        print(f"\"So you wont even deny these rumours huh {player}? Well at least you didn't lie to me dear.\" {maingirl} released {player} \"Well aren't you a deviant my dear.\"\n")
    elif any(choice in ("b", "d") for choice in counter_All): 
        print(f"\"Well, your not the most honest one ain't ya {player}? Dont be shy with me now dear.\" {maingirl} bopped {player} on the nose.\n")
    else:
        print(f"\"You dont have to lie to me now dear, I don't really care if so long as you explain yourself properly and repent.\" {maingirl} flicked {player} on the forehead.\n")
        print(f"\"Aw\" {player} rubbed his forehead with a bit of shame.\n")
    sleep(10)
    print(
